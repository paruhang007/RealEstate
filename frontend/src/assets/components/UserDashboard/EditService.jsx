...
export default function AddService() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (imageLink) {
      try {
        const response = await fetch("http://localhost:4000/editService", {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          <HStack gap={5} align={"center"} mt={5}>
            <Box>
              ...
              >
                <option
                  value="Hardware"
                  selected={service.selectedServiceType === "Hardware"}
                >
                  Hardware Store
                </option>
                <option
                  value="Plumber"
                  selected={service.selectedServiceType === "Plumber"}
                >
                  Plumber
                </option>
                <option
                  value="Constructions"
                  selected={service.selectedServiceType === "Constructions"}
                >
                  Constructions
                </option>
                <option
                  value="Electrician"
                  selected={service.selectedServiceType === "Electrician"}
                >
                  Electrician
                </option>
                <option
                  value="Carpet"
                  selected={service.selectedServiceType === "Carpet"}
                >
                  Carpet Fitting
                </option>
                <option
                  value="Marbles"
                  selected={service.selectedServiceType === "Marbles"}
                >
                  Marbles and Tiles
                </option>
                <option
                  value="Furniture"
                  selected={service.selectedServiceType === "Furniture"}
                >
                  Furniture
                </option>
                <option
                  value="Solar"
                  selected={service.selectedServiceType === "Solar"}
                >
                  Solar Heaters
                </option>
                <option
                  value="Metal"
                  selected={service.selectedServiceType === "Metal"}
                >
                  Metal Work
                </option>
                <option
                  value="Paint"
                  selected={service.selectedServiceType === "Paint"}
                >
                  Paint Store
                </option>
                <option
                  value="Cleaning"
                  selected={service.selectedServiceType === "Cleaning"}
                >
                  Cleaning Service
                </option>
                <option
                  value="Interior"
                  selected={service.selectedServiceType === "Interior"}
                >
                  Interior Design
                </option>
              </Select>
          ...
          <HStack gap={5} align={"center"} mt={5}>
            ...
            <Box>
              <FormControl id="tol" isRequired>
                ...
                <Input
                  type="text"
                  defaultValue={service.serStreet}
                  onChange={(e) => {
