		...
		if (search.length === 0) {
			...
			setSelectedPropertyType(
				property.filter((prop) => {
					return prop.package.propName.toLowerCase().includes(search.toLowerCase()) || prop.package._id.toLowerCase().includes(search.toLowerCase());
				})
			);
		}
	};
	...
	const handlerCate = (e) => {
		...
		if (select === "all") {
			...
			setSelectedPropertyType(
				selectedFor !== "all"
					? property.filter((prop) => {
						return prop.package.selectedFor === selectedFor && prop.package.selectedPropertyType === select;
					})
					: property.filter((prop) => {
	...
	const handleLeige = (e) => {
		...
		if (select === "all") {
			...
			setSelectedPropertyType(
				selectedType !== "all"
					? property.filter((prop) => {
						return prop.package.selectedFor === select && prop.package.selectedPropertyType === selectedType;
					})
					: property.filter((prop) => {
